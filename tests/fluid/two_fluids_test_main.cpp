...
/***************************************************** 
 *
 * Test suite for the Fluid class - two fluids
 *
 *****************************************************/
...
bool check_from_files(const std::string&, const double, const Geometry&, const int);
bool check_values(const std::string&, const double, const Geometry&);
bool check_equilibrium_distribution(Fluid& fluid, const size_t Nx, const size_t Ny);
...
bool empty_geom()
{
	...
	Fluid water;
	Fluid oil;
	water.simple_ini(geom, rho_0, mcmp_sys);
	oil.simple_ini(geom, rho_0, mcmp_sys);

	// Output files (each fluid will have _1 or _2 added to it)
	std::string den_file{"test_data/mcmp_macro_density"};
	std::string ux_file{"test_data/mcmp_macro_vel_x"};
	std::string uy_file{"test_data/mcmp_macro_vel_y"};
	std::string den_dist_file{"test_data/mcmp_den_dist"};

	// The macroscopic density should be 2, velocities 0.0, and 
	// the distribution function should be 2/9 everywhere
	water.save_state(den_file + "_1", ux_file + "_1", uy_file + "_1", 10, geom);
	water.write_f(den_dist_file + "_1");	
	oil.save_state(den_file + "_2", ux_file + "_2", uy_file + "_2", 10, geom);
	oil.write_f(den_dist_file + "_2");

	if (!check_from_files("test_data/mcmp_", rho_0, geom, n_fluids)) {
		std::cerr << "Wrong initial properties" << std::endl;
		return false;
	}
	if (!check_equilibrium_distribution(water, Nx, Ny)) {
		std::cerr << "Wrong zero velocity equlibrium density distribution for fluid water" << std::endl;
		return false;
	}
	if (!check_equilibrium_distribution(oil, Nx, Ny)) {
		std::cerr << "Wrong zero velocity equlibrium density distribution for fluid oil" << std::endl;
		return false;
	}
	return true;
}
...
bool fluid_with_walls()
{
	...
	// Output files
	std::string den_file{"test_data/mcmp_wt_macro_density"};
	std::string ux_file{"test_data/mcmp_wt_macro_vel_x"};
	std::string uy_file{"test_data/mcmp_wt_macro_vel_y"};
	std::string den_dist_file{"test_data/mcmp_wt_den_dist"};
	...
	if (!check_from_files("test_data/mcmp_wt_", rho_0, geom, n_fluids)) {
		std::cerr << "Wrong initial properties" << std::endl;
		return false;
	}
	if (!check_equilibrium_distribution(water, Nx, Ny)) {
...
bool object_array()
{
	...
	// Output files
	std::string den_file{"test_data/mcmp_oat_macro_density"};
	std::string ux_file{"test_data/mcmp_oat_macro_vel_x"};
	std::string uy_file{"test_data/mcmp_oat_macro_vel_y"};
	std::string den_dist_file{"test_data/mcmp_oat_den_dist"};
	...
	if (!check_from_files("test_data/mcmp_oat_", rho_0, geom, n_fluids)) {
		std::cerr << "Wrong initial properties" << std::endl;
		return false;
	}
	if (!check_equilibrium_distribution(water, Nx, Ny)) {
...
/// @details The last parameter is the number of fluids
bool check_from_files(const std::string& fname, const double exp_value, 
						const Geometry& geom, const int n_fluids)
{
	for (int fli = 1; fli <= n_fluids; ++fli) {
		// Macroscopic density
		if (!check_values(fname + "macro_density_" + std::to_string(fli) + "_10.txt", exp_value, geom)) {
			return false;
		}	
		// Macroscopic x velocity
		if (!check_values(fname + "macro_vel_x_" + std::to_string(fli) + "_10.txt", 0.0, geom)) {
			return false;
		}
		// Macroscopic y velocity
		if (!check_values(fname + "macro_vel_y_" + std::to_string(fli) +"_10.txt", 0.0, geom)) {
			return false;
		}
		// Density distribution
		const double dist_exp_value = exp_value/9.0;
		for (int fi = 0; fi<9; ++fi) {
			if (!check_values(fname + "den_dist_" + std::to_string(fli) + "_" + std::to_string(fi) + ".txt", dist_exp_value, geom)) {
				return false;
			}	
		}
	}
