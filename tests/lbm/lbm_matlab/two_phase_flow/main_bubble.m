...
Len_Channel_2D=99; 
Channel_2D_half_width=100; Width=Channel_2D_half_width*2;
% Fluid area
...
bC=100; 
density_1(39:79,bC-20:bC+20)=2; 
density_2(find(density_1==0&Channel2D~=0))=2; 
density_temp=density_1;
% Exaggerated
density_1(find(density_2~=0&Channel2D~=0))=0.6;
density_2(find(density_temp~=0&Channel2D~=0))=0.6;
...
while (~StopFlag)
   ...
   if(labindex==1)
    ...
    if (mod(Cur_Iter,save_every)==0)
        ...
        feq_1=zeros(Nr,Mc,Nc);
        feq_2=zeros(Nr,Mc,Nc);
        ...
        for kCore=2:nCores
            if(labindex==kCore)           
                ...
                labSend(PARTITIONS.(part_names{kCore}).feq_1,1);
                labSend(PARTITIONS.(part_names{kCore}).feq_2,1);

                labSend(PARTITIONS.(part_names{kCore}).Fx_1,1);
                labSend(PARTITIONS.(part_names{kCore}).Fx_2,1);
                labSend(PARTITIONS.(part_names{kCore}).Fy_1,1);
                labSend(PARTITIONS.(part_names{kCore}).Fy_2,1);

                labSend(PARTITIONS.(part_names{kCore}).ucx_1,1);
                labSend(PARTITIONS.(part_names{kCore}).ucx_2,1);
                labSend(PARTITIONS.(part_names{kCore}).ucy_1,1);
                labSend(PARTITIONS.(part_names{kCore}).ucy_2,1);
            end
            if(labindex==1)
                f_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                f_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);

                feq_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                feq_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);
                 
                Fx_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                Fx_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);
                Fy_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                Fy_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);

                ucx_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                ucx_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);
                ucy_1((kCore-1)*Nprt:(kCore)*Nprt,:,:)=labReceive(kCore);
                ucy_2((kCore-1)*Nprt:kCore*Nprt,:,:)=labReceive(kCore);
            end
        end
        ...
        if(labindex==1)
            ...
            feq_1(1:(Nprt-1),:,:)=PARTITIONS.partition_1.feq_1((Nprt+2):1:(end-1),:,:);
            feq_1((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.feq_1(2:(Nprt+1),:,:);
            
            feq_2(1:(Nprt-1),:,:)=PARTITIONS.partition_1.feq_2((Nprt+2):1:(end-1),:,:);
            feq_2((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.feq_2(2:(Nprt+1),:,:);

            Fx_1(1:(Nprt-1),:,:)=PARTITIONS.partition_1.Fx_1((Nprt+2):1:(end-1),:,:);
            Fx_1((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.Fx_1(2:(Nprt+1),:,:);
            Fx_2(1:(Nprt-1),:,:)=PARTITIONS.partition_1.Fx_2((Nprt+2):1:(end-1),:,:);
            Fx_2((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.Fx_2(2:(Nprt+1),:,:);
            Fy_1(1:(Nprt-1),:,:)=PARTITIONS.partition_1.Fy_1((Nprt+2):1:(end-1),:,:);
            Fy_1((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.Fy_1(2:(Nprt+1),:,:);
            Fy_2(1:(Nprt-1),:,:)=PARTITIONS.partition_1.Fy_2((Nprt+2):1:(end-1),:,:);
            Fy_2((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.Fy_2(2:(Nprt+1),:,:);
            
            ucx_1(1:(Nprt-1),:,:)=PARTITIONS.partition_1.ucx_1((Nprt+2):1:(end-1),:,:);
            ucx_1((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.ucx_1(2:(Nprt+1),:,:);
            ucx_2(1:(Nprt-1),:,:)=PARTITIONS.partition_1.ucx_2((Nprt+2):1:(end-1),:,:);
            ucx_2((nCores*Nprt+1):Nr,:,:)=PARTITIONS.partition_1.ucx_2(2:(Nprt+1),:,:);
            ucy_1(1:(Nprt-1),:,:)=PARTITIONS.partition_1.ucy_1((Nprt+2):1:(end-1),:,:);
