...
def load_and_compare(cppname, mname, var, tres = ''):
    ...
    if not np.allclose(msol, cppsol, 1e-5, 1e-4):
        print('Cpp', var, 'not equal to matlab one - reducing tolerance')
        if not np.allclose(msol, cppsol, 1e-3, 1e-2):
            print('Cpp', var, 'not equal to matlab one with reduced tolerance')
            return False 
    return True
